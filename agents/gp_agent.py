...
class GPAgent(BaseAgent):
    def __init__(self,
        ...
        # gp layer
        self.gp_layer = StandardGPModel(train_x=None, train_y=None, likelihood=self.likelihood, kernel_type='linear')
        self.config = config
    ...
    def update_gp(self, induce_batch):
        states, actions, rewards, next_states, dones = induce_batch
        # states: shape [batch_size x state_dim]
        states = torch.tensor(states, dtype=torch.float32).to(self.device)
